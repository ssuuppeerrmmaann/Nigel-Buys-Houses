...
export default function App() {
  ...
  // Interactive Map / Coordinator Search tab state
  ...
  // FAQ Expanded accordion indices
  const [expandedFaq, setExpandedAccordion] = useState<number | null>(null);

  // Admin Leads database simulation in LocalStorage
  ...
  const handleLeadSubmit = (e: React.FormEvent) => {
    ...
    if (!phone.trim()) {
      setFieldError('A valid cell phone number is required so our local coordinator can call.');
      return;
    }
  ...
  const filteredStates = STATE_RECORDS.filter(s => 
    s.name.toLowerCase().includes(mapSearch.toLowerCase()) ||
    s.coordinator.toLowerCase().includes(mapSearch.toLowerCase())
  );

  // Dynamic calculated options
  // 1. Direct cash offer: is usually around 70-80% of retail minus repairs, but zero fees
  const cashOfferPercentage = 0.76; 
  const cashOfferValue = Math.round(houseValue * cashOfferPercentage - (repairEstimate * 0.5));
  
  // 2. Traditional listing costs
  ...
  const traditionalHoldingCosts = Math.round(houseValue * 0.02); // 2% holding costs over 6 months
  const traditionalNetP_ayout = houseValue - traditionalCommission - traditionalClosingCosts - traditionalRepairs - traditionalHoldingCosts;

  // 3. Retail Partnership Option: sell closer to retail, minor fees, we cover upfront coordination
  const partnerSalePercentage = 0.94;
  const partnerListingPrice = houseValue;
  const partnerSaleRepairs = Math.round(repairEstimate * 0.2); // minimally repairs to list
  const partnerClosingCosts = Math.round(houseValue * 0.01); 
  const partnerListingFee = Math.round(houseValue * 0.025); // low overhead listing/match fee
    ...
    <div className="min-h-screen flex flex-col font-sans transition-colors duration-300">
      ...
      <header className="bg-white border-b border-[#ced1d5]/40 sticky top-0 z-40 transform transition-all">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between">
          ...
          {/* RIGHT ACTION CORNER - DYNAMIC TEL */}
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-[#ced1d5]/40 bg-slate-50 px-4 py-4 space-y-3.5 shadow-inner">
            ...
            <a 
              ...
              className="block px-3 py-2.5 rounded-lg font-semibold text-slate-800 hover:bg-[#ff7043]/10 hover:text-[#ff7043]"
            >
              How It Works
            </a>
            <a 
            ...
            <div className="pt-4 border-t border-slate-200 flex flex-col space-y-2">
              ...
              <a 
                href="tel:(480)500-9801" 
                className="flex items-center justify-center p-3 text-lg font-black bg-[#ff7043] text-white rounded-lg shadow"
      ...
      {/* HERO SECTION WITH MULTI-STEP CAPTURE */}
      <section className="relative bg-slate-900 text-white overflow-hidden py-14 lg:py-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          {/* HERO RIGHT: MULTI-STEP LEAD INTERACTIVE CAPTURE FORM */}
          <div className="lg:col-span-5 bg-white rounded-2xl p-6 md:p-8 shadow-2xl text-[#092641] relative border-b-4 border-[#ff7043]">
            ...
            {step === 1 && (
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input 
                        ...
                        placeholder="Orlando" 
                        className="w-full px-3 py-2.5 border-2 border-slate-200 rounded-lg text-sm focus:border-[#ff7043] focus:outline-none"
                      />
                    </div>
                    <div>
                ...
                <button 
                  ...
                  className="w-full mt-4 bg-[#ff7043] hover:bg-[#e65100] text-white py-3 rounded-lg font-serif text-base font-bold shadow-md hover:shadow-lg transition flex items-center justify-center space-x-1"
                  style={{ minHeight: '44px' }}
                >
            ...
            {step === 2 && (
              <form onSubmit={handleLeadSubmit} className="space-y-4">
                <div className="text-center lg:text-left">
                  <h3 className="text-lg md:text-xl font-bold font-serif mb-1 text-[#092641]">Who Should We Send This Offer To?</h3>
                  <p className="text-xs text-[#868c92] font-semibold">We analyze County assessor records live. Provide correct details.</p>
                </div>
                ...
                <div className="space-y-3.5 pt-1">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      placeholder="e.g. (480) 500-9801" 
                      className="w-full px-3 py-2.5 border-2 border-[#ced1d5] rounded-lg text-sm focus:border-[#ff7043] focus:outline-none"
                    />
                    <span className="text-[10px] text-slate-400 mt-1 block">To confirm your ownership and schedule a call.</span>
                  </div>

                  <div>
                    <label className="block text-xs font-bold uppercase text-[#092641] mb-1">Email <span className="text-slate-400">(optional)</span></label>
                    <input 
                      type="email" 
                      value={email}
                ...
                <div className="flex space-x-3.5 pt-2">
                  <button 
                    type="button"
                    onClick={() => setStep(1)}
                    className="w-1/3 bg-slate-100 hover:bg-slate-200 text-slate-700 py-3 rounded-lg text-sm font-bold transition"
                    style={{ minHeight: '44px' }}
                  >
                    Back
                  </button>
                  ...
                  <button 
                    ...
                    className="w-2/3 bg-[#ff7043] hover:bg-[#e65100] text-white py-3 rounded-lg font-serif text-base font-bold shadow-md hover:shadow-lg transition flex items-center justify-center space-x-2"
                    style={{ minHeight: '44px' }}
                  >
                    {isSubmitting ? (
                      <>
            ...
            {/* STEP 3: SUCCESS ESTIMATE GENERATION */}
            {step === 3 && successLead && (
              <div className="space-y-4 text-center">
                ...
                <button 
                  ...
                  className="w-full bg-[#092641] hover:bg-[#1e3a5f] text-white py-3 rounded-lg text-sm font-extrabold shadow transition"
                  style={{ minHeight: '44px' }}
                >
            ...
            {/* TRUST SUB-TEXT / PRIVACY PROTECTION */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-[#f8fafc] rounded-2xl p-8 border border-slate-100 relative group hover:shadow-lg transition">
              <div className="absolute top-0 right-8 translate-y-[-50%] bg-[#ff7043] text-white font-serif font-bold text-lg pt-1 pb-1.5 px-3.5 rounded-full inline-block">
                1
              </div>
              <div className="text-[#ff7043] bg-white p-3 rounded-xl shadow inline-block mb-6">
              ...
              <p className="text-slate-500 text-sm font-light leading-relaxed">
                Fill out the quick valuation form above, or call our 24/7 coordinator line. It is easy, completely free, and carries zero obligation.
              </p>
            </div>
            ...
            <div className="bg-[#f8fafc] rounded-2xl p-8 border border-slate-100 relative group hover:shadow-lg transition">
              ...
              <p className="text-slate-500 text-sm font-light leading-relaxed">
                We present two concrete choices: A fast cash buyout quote directly from us, or our popular facilitated retail list program targeting top value.
              </p>
            </div>
            ...
            <div className="bg-[#f8fafc] rounded-2xl p-8 border border-slate-100 relative group hover:shadow-lg transition">
              ...
              <p className="text-slate-500 text-sm font-light leading-relaxed">
                If you accept our buy proposal, select your target closing date. You get paid in 10-14 days. We handle paperwork, cleanup, and closing costs.
              </p>
            </div>
          ...
          <div className="mt-12 text-center">
            <a 
              href="#sticky-callout"
              className="inline-flex items-center space-x-2 bg-[#092641] hover:bg-[#1e3a5f] text-white py-3.5 px-8 rounded-full font-serif font-bold shadow-md transition"
            >
      ...
      <section id="compare" className="py-20 bg-slate-50 border-y border-[#ced1d5]/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
            ...
            <p className="text-slate-600 font-light">
              We believe in "People Over Profits." Unlike generic real estate investors, we provide multiple choices so you can contrast what works best.
            </p>
          </div>

          {/* TWO MAIN PROGRAM DESCRIPTION CARDS */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
            
            {/* Card 1: Direct Cash buy option */}
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border-t-8 border-[#ff7043] hover:shadow-md transition">
              <div className="flex justify-between items-center mb-6">
                <span className="bg-[#ff7043]/10 text-[#ff7043] px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                  Option A
                </span>
                <span className="text-xs text-[#868c92] font-semibold tracking-wider flex items-center">
                  <Clock className="h-4 w-4 mr-1 text-[#ff7043]" />
                  9-14 Days Close
                </span>
              </div>
              ...
              <ul className="space-y-3.5 text-sm font-medium text-slate-700">
                <li className="flex items-start space-x-2">
                  ...
                  <span><strong>Zero cleaning required:</strong> Sell in exact as-is condition. Leave unwanted property items.</span>
                </li>
                <li className="flex items-start space-x-2">
                  <Check className="h-5 w-5 text-emerald-500 shrink-0 mt-0.5" />
                  <span><strong>No repairs:</strong> Skip structural fix costs, painting, mold work, or exterior updates.</span>
                </li>
                <li className="flex items-start space-x-2">
                  <Check className="h-5 w-5 text-emerald-500 shrink-0 mt-0.5" />
                  <span><strong>No listing delays:</strong> Get paid quickly with our assured funds. Avoid long banks mortgage loans.</span>
                </li>
                <li className="flex items-start space-x-2">
                  <Check className="h-5 w-5 text-emerald-500 shrink-0 mt-0.5" />
                  <span><strong>Zero fees:</strong> No real estate agent commissions or hidden closing fee structures.</span>
                </li>
              </ul>
            </div>

            {/* Card 2: Almost Retail Option */}
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border-t-8 border-[#213d55] hover:shadow-md transition">
              <div className="flex justify-between items-center mb-6">
                <span className="bg-slate-100 text-slate-700 px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                  Option B
                </span>
                <span className="text-xs text-[#868c92] font-semibold tracking-wider flex items-center">
              ...
              <h3 className="text-2xl font-serif font-black text-[#092641] mb-3">
                The "Almost Retail" Partnership
              </h3>
              <p className="text-slate-500 text-sm font-light mb-6">
                Keep the majority of your hard-earned equity. We match your property with our localized trained buyer network who are willing to purchase your home with minimal repair expectations.
              </p>

              <ul className="space-y-3.5 text-sm font-medium text-slate-705">
                <li className="flex items-start space-x-2">
                  <Check className="h-5 w-5 text-emerald-500 shrink-0 mt-0.5" />
                  <span><strong>Highest Payout:</strong> Payout matches close to standard market value.</span>
                </li>
                <li className="flex items-start space-x-2">
                  <Check className="h-5 w-5 text-emerald-500 shrink-0 mt-0.5" />
                  <span><strong>Upfront Coordination:</strong> We handle incoming inquiries and vet potential buyers.</span>
                </li>
                <li className="flex items-start space-x-2">
                  <Check className="h-5 w-5 text-emerald-500 shrink-0 mt-0.5" />
                  <span><strong>Lower Headaches:</strong> Way faster and simpler than traditional listing with standard broker contracts.</span>
                </li>
                <li className="flex items-start space-x-2">
                  <Check className="h-5 w-5 text-emerald-500 shrink-0 mt-0.5" />
                  <span><strong>Minimal standard fees:</strong> Avoid typical realtor commission structures of 6%.</span>
                </li>
              </ul>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 md:p-10 shadow-lg border border-[#ced1d5]/40 max-w-5xl mx-auto">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between border-b border-slate-100 pb-6 mb-8 gap-4">
              <div className="space-y-1">
                <span className="text-xs font-extrabold text-[#ff7043] uppercase tracking-widest flex items-center">
                  <Calculator className="h-4 w-4 mr-1" />
                  Lead Equity Tool
                </span>
                <h3 className="text-xl md:text-2xl font-serif font-black text-[#092641]">
                  Interactive Net Payout Calculator
                </h3>
                <p className="text-slate-500 text-xs font-light">
                  Input your home values to contrast direct payout vs brokerage expenses.
                </p>
              </div>
              ...
              <div className="w-full lg:w-96 text-left space-y-1">
                ...
                <input 
                  ...
                  step="10000"
                  value={houseValue}
                  onChange={(e) => setHouseValue(parseInt(e.target.value))}
                  className="w-full h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-[#ff7043]"
                />
              ...
              <div className="lg:col-span-4 space-y-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setRepairEstimate(Math.max(0, parseInt(e.target.value) || 0))}
                    className="w-full px-3 py-2 border-2 border-slate-200 rounded-lg text-sm focus:border-[#ff7043] focus:outline-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setAgentCommissionPct(Math.max(0, parseFloat(e.target.value) || 0))}
                      className="w-full px-3 py-2 border-2 border-slate-200 rounded-lg text-sm focus:border-[#ff7043] focus:outline-none"
                    />
                  </div>
                  <div>
              ...
              <div className="lg:col-span-8 bg-slate-900 text-white rounded-xl p-6 relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
                  ...
                  <div className="bg-slate-800/80 border border-slate-700 rounded-xl p-4 text-center">
                    ...
                    <span className="block text-xl font-bold mt-1 text-slate-300">${traditionalNetP_ayout.toLocaleString()}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
            ...
            <div className="lg:col-span-5 flex flex-col space-y-4">
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <input 
                  type="text"
                  placeholder="e.g. Texas, Florida, Michigan..."
                  value={mapSearch}
                  onChange={(e) => setMapSearch(e.target.value)}
                  className="w-full px-3 py-2 border-2 border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:border-[#ff7043]"
                />
              </div>

              {/* Scrollable checklist of active states */}
              <div className="border border-slate-100 rounded-xl overflow-y-auto max-h-96 divide-y divide-slate-100">
                {filteredStates.map((st) => (
                  <button 
                    key={st.abbr}
                    onClick={() => handleStateAbbrSelect(st.abbr)}
                    className={`w-full text-left px-4 py-3 flex justify-between items-center transition ${activeStateAbbr === st.abbr ? 'bg-[#ff7043]/10 font-bold border-l-4 border-[#ff7043]' : 'hover:bg-slate-50'}`}
                  >
                    <div className="flex items-center space-x-2.5">
                      <span className="inline-block bg-[#092641] text-white text-[10px] w-5 h-5 rounded flex items-center justify-center font-bold tracking-tight">
                        {st.abbr}
                      </span>
                ...
                {filteredStates.length === 0 && (
                  <div className="p-8 text-center text-[#868c92] text-sm">
                    No matching states found. Try typing a simpler key.
                  </div>
                )}
              </div>
            </div>

            {/* Right: Interactive State Payout Coordinator Showcase Panel */}
            <div className="lg:col-span-7 bg-[#092641] text-white rounded-2xl p-6 md:p-8 relative overflow-hidden flex flex-col justify-between">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-700 pb-4 gap-2">
                  <div>
                    <h3 className="text-2xl md:text-3xl font-serif font-bold text-white tracking-tight flex items-center">
                      <MapPin className="stroke-2 text-[#ff7043] mr-2 h-6 w-6 shrink-0" />
                      Active: {activeStateRecord.name}
                    </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-3.5">
                      <div className="h-12 w-12 rounded-full bg-slate-800 flex items-center justify-center font-bold text-xl text-[#ff7043] border border-slate-700">
                        {activeStateRecord.coordinator.charAt(0)}
                      </div>
                      <div>
                    ...
                    <div className="space-y-2 pt-2 text-sm text-slate-300">
                      <div className="flex justify-between py-1 border-b border-slate-800">
                        ...
                        <strong className="text-white">{activeStateRecord.speed}</strong>
                      </div>
                      <div className="flex justify-between py-1 border-b border-slate-800">
                        <span>Active Underwriters / Buyers:</span>
                        <strong className="text-white">{activeStateRecord.activeBuyers} partners</strong>
                      </div>
                      <div className="flex justify-between py-1 border-b border-slate-800">
                        <span>Lead Rating in {activeStateAbbr}:</span>
                        <strong className="text-amber-400 flex items-center">
                          ★ {activeStateRecord.rating} / 5.0
                        </strong>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-800/50 border border-slate-800 rounded-xl p-5 flex flex-col justify-between">
                    ...
                    <div className="mt-4 pt-3.5 border-t border-slate-800 flex justify-between items-center text-[11px] text-slate-400">
                      <span>&mdash; Verified {activeStateAbbr} Seller Review</span>
                      <strong className="text-white">Rated A+</strong>
                    </div>
                  </div>
                </div>
              ...
              {/* Coordinator Payout trigger button */}
              <div className="mt-8 pt-4 border-t border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4">
                <p className="text-xs text-slate-400 text-center sm:text-left">
                  Need an estimate in <strong>{activeStateRecord.name}</strong>? Lock your local coordinator rate today.
                </p>
                <a 
                  href="#sticky-callout" 
                  className="bg-[#ff7043] hover:bg-[#e65100] text-white px-6 py-2.5 rounded-lg text-sm font-bold tracking-tight shadow flex items-center justify-center shrink-0 w-full sm:w-auto"
                  style={{ minHeight: '44px' }}
                >
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <strong className="block text-white text-sm">Transparency Standard</strong>
                    <span className="text-slate-400 text-xs">Clear calculations, local county records matched honestly.</span>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <strong className="block text-white text-sm">Empowerment Education</strong>
                    <span className="text-slate-400 text-xs">We tell you when Listing traditionally is more profitable.</span>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <strong className="block text-white text-sm">Zero Obligation Lock</strong>
                    <span className="text-slate-400 text-xs">An estimate does not obligate you. Take up to 30 days to reject.</span>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <strong className="block text-white text-sm">Decades of Local Depth</strong>
                    <span className="text-slate-400 text-xs">We partner with native escrow title companies.</span>
                  </div>
                </div>
              </div>
            </div>

            {/* Right: Core Values Table Card */}
            <div className="lg:col-span-5 bg-slate-800 border border-slate-700 rounded-2xl p-6 relative">
              <h3 className="text-lg font-serif font-black text-white mb-4 flex items-center">
                <Award className="text-[#ff7043] mr-2 h-5 w-5 stroke-2" />
                Our Operational Code
              </h3>
              
              <div className="divide-y divide-slate-700 text-xs text-slate-300">
                <div className="py-3 flex justify-between">
                  <span className="font-bold text-white">✔ Integrity First</span>
                  <span className="text-right text-slate-400">Do right even when unmonitored</span>
                </div>
                <div className="py-3 flex justify-between">
                  <span className="font-bold text-white">✔ Transparent Calculations</span>
                  <span className="text-right text-slate-400">Zero hidden fees or list surprises</span>
                </div>
                <div className="py-3 flex justify-between">
                  <span className="font-bold text-white">✔ No-Pressure Guidance</span>
                  <span className="text-right text-slate-400">Empower, never coerce or lock</span>
                </div>
                <div className="py-3 flex justify-between">
                  <span className="font-bold text-white">✔ Support Local Vets</span>
                  <span className="text-right text-slate-400">A portion of profits go to veteran houses</span>
                </div>
                <div className="py-3 flex justify-between">
                  <span className="font-bold text-white">✔ Win-Win Payouts</span>
                  <span className="text-right text-slate-400">If it doesn't solve your query, reject</span>
                </div>
              </div>

              <div className="mt-6 text-center text-[10px] text-slate-500 italic">
                A+ BBB Rated Real Estate Investor standard representation code.
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center space-y-4 mb-14">
            ...
            <p className="text-slate-600 font-light text-sm">
              Do you have a question about how we valuate? Here are candid answers based on 9,000+ real inquiries.
            </p>
          </div>
          ...
          <div className="space-y-4">
            {[
              ...
              {
                q: "How exactly do you calculate my payout offer?",
                a: "Our formula is direct and transparent. We look at: 1) Local active comparable sales (comps) in your immediate county, 2) Necessary structural repair estimates, 3) Intrinsic holding fees. We combine this into a quick direct cash buyout option or our Retail Partnership program."
              },
              {
                q: "Is there any obligation when I submit my home address?",
                a: "No! Accessing your layout valuation carries zero obligation. We run state records for you free of charge. You can review our estimates with family, compare against standard agents, or discard completely."
              },
              {
              ...
              },
              {
                ...
                a: "We purchase in exact as-is condition. You do not need to clean closets, pick up trash, repair roofing, or paint walls. We absorb those coordination efforts fully within our buyout structures."
              },
              {
                q: "How fast can you buy my house and make the payout?",
                a: "Once you approve the direct buy offer, we can close the escrow processing in 9 to 14 business days. Payout is coordinated directly through a local certified bank wire or bank-guaranteed cashier checks."
              }
            ].map((faq, index) => (
              <div 
                key={index} 
                className="bg-white rounded-xl border border-[#ced1d5]/40 overflow-hidden shadow-xs hover:border-[#ff7043]/30 transition"
              >
                <button 
                  onClick={() => setExpandedAccordion(expandedFaq === index ? null : index)}
                  className="w-full text-left px-5 py-4 flex items-center justify-between font-bold text-[#092641] text-sm md:text-base"
                  style={{ minHeight: '44px' }}
                >
                  <span>{faq.q}</span>
                ...
                {expandedFaq === index && (
                  <div className="px-5 pb-5 pt-1 text-slate-600 font-light text-sm border-t border-slate-50 leading-relaxed bg-[#f8fafc]">
                    {faq.a}
                  </div>
                )}
              </div>
      ...
      <footer className="bg-white border-t border-[#ced1d5]/40 py-16 text-slate-600 text-xs md:text-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-12 gap-10">
          ...
          {/* Coordinator Hotline column */}
          <div className="md:col-span-3 space-y-3">
            <h4 className="font-bold text-[#092641] uppercase tracking-wider text-[11px]">Help Hotline</h4>
            <div className="space-y-2">
              <a href="tel:(480)500-9801" className="font-bold text-[#ff7043] text-sm hover:underline block">
                ☎ Call/Text: (480) 500-9801
              </a>
          ...
          {/* Interactive Portal links */}
          <div className="md:col-span-3 space-y-3">
            <h4 className="font-bold text-[#092641] uppercase tracking-wider text-[11px]">Testing Tools</h4>
            <div className="space-y-2">
              <button 
                onClick={() => setAdminOpen(true)}
                className="bg-[#092641] text-white px-3.5 py-1.5 rounded text-[11px] font-bold uppercase tracking-wider flex items-center space-x-1.5 hover:bg-[#1e3a5f] transition cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden text-[#092641] flex flex-col max-h-[90vh]">
            ...
            <div className="bg-[#092641] text-white p-5 flex justify-between items-center">
              <div>
                <h3 className="text-lg font-serif font-bold text-white flex items-center">
                  <ClipboardList className="h-5 w-5 text-[#ff7043] mr-2" />
                  Lead Valuation Inspect Portal (Admin Simulator)
                </h3>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                <div className="relative w-full sm:w-72">
                  <input 
                    ...
                    value={adminSearch}
                    onChange={(e) => setAdminOpenSearch(e.target.value)}
                    className="w-full pl-3 pr-3 py-1.5 border border-slate-300 bg-white rounded text-xs focus:outline-none"
                  />
                </div>
                <div className="text-xs text-slate-500 font-bold shrink-0">
                  Total Leads Persistent: {leadsDb.length}
                </div>
              </div>

              {/* Grid or Table list of leads */}
              {leadsDb.filter(l => 
                l.firstName.toLowerCase().includes(adminSearch.toLowerCase()) ||
                l.lastName.toLowerCase().includes(adminSearch.toLowerCase()) ||
                l.address.toLowerCase().includes(adminSearch.toLowerCase())
              ).map((lead) => {
                    ...
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b border-slate-100 pb-2 gap-2">
                      ...
                      <div className="flex items-center space-x-2">
                        <select 
                          ...
                          className={`text-xs font-bold px-2.5 py-1 rounded-full border bg-white ${
                            lead.status === 'New' ? 'text-blue-600 border-blue-200' :
                            lead.status === 'Analyzing' ? 'text-amber-600 border-amber-200' :
                            lead.status === 'Offer Sent' ? 'text-green-600 border-green-200' :
                        ...
                        <button 
                          ...
                          className="text-[#ff7043] hover:text-red-600 p-1 hover:bg-slate-100 rounded"
                          title="Delete record"
              ...
              {leadsDb.length === 0 && (
                <div className="text-center p-12 bg-slate-50 rounded-xl">
                  ...
                  <p className="text-sm font-bold text-slate-500">No home valuation lead records currently saved.</p>
                  <p className="text-xs text-slate-400">Use the Hero Valuation widget to save your first lead record!</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-100 flex justify-between items-center text-xs">
              <span className="text-slate-400">Persisted locally inside this preview session.</span>
              <button 
                onClick={() => setAdminOpen(false)}
                className="bg-[#092641] text-white hover:bg-slate-800 px-5 py-2 rounded font-bold"
                style={{ minHeight: '36px' }}
              >
                Close View
              </button>
